...
- (void)genericApiMethod:(apiMethods)apiMethod withUrlSegments:(NSArray *)urlSegments withQueryString:(NSString *)queryString withPostData:(id)postData
{
    ...
    else if ([[ReachabilityManager sharedInstance] isTheApiReachable] == YES || [self.delegate isKindOfClass:[RegisterUserViewController class]])
    {
        ...
        // In a truly object-oriented program, this would be a property of some object, rather than just a local variable
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveResponse:(NSURLResponse *)response completionHandler:(void (^)(NSURLSessionResponseDisposition))completionHandler
{
    ...
    // Note this filters out Equipment create requests (POSTs), for which we need to parse the returned object to obtain the new Id
    // We also exclude Test Session updates, as we need to check whether the test session is now complete so that the local documents can be cleared out
    if (
        [self.delegate isKindOfClass:[SyncManager class]] &&
        (_httpMethod == PUT || _apiMethod == EquipmentUploadUpdate) &&
        !(_httpMethod == PUT && _apiMethod == TestSessionsLite) &&
        [[NSNumber numberWithInteger:[httpResponse statusCode]] isHttpSuccessStatusCode]
        )
    {
        [dataTask cancel];
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (parsedData == nil)
                ...
                if (_statusCode && _statusCode == 409 && [self.delegate respondsToSelector:@selector(didReportConflictWithData:)])
                    ...
                    // Success response
                    // If this is a Test Sessions update, we also add in the status code so the SyncManager can verify its success
                    if (_httpMethod == PUT && _apiMethod == TestSessionsLite) {
                        NSMutableDictionary *parsedDataMutable = [NSMutableDictionary dictionaryWithDictionary:parsedData];
                        [parsedDataMutable setObject:[NSNumber numberWithInteger:_statusCode] forKey:API_STATUS_CODE];
                        parsedData = [NSDictionary dictionaryWithDictionary:parsedDataMutable];
                    }
                    // Call the standard data handling delegate method
