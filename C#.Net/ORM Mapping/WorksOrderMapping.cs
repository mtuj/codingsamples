...
namespace Vision.Api.DotNet.Domain.NHibernate.Mapping.WorksOrders
{
    public class WorksOrderMapping : ClassMapping<WorksOrder>
    {
        public WorksOrderMapping()
        {
            ...
            //ManyToOne(x => x.ResponsibleUnit, map => map.Column("ResponsibleUnit"));
            ...
            Property(x => x.RetentionAmount, map => map.Formula(@"(SELECT (SELECT COALESCE(SUM(CASE WHEN _it.[Name] = 'Credit Note' THEN (0 - _il.[RetentionAmount]) ELSE _il.[RetentionAmount] END), 0)
		                                                            FROM
			                                                            dbo.[InvoiceLine] _il
			                                                            INNER JOIN
			                                                            dbo.[Invoice] _i ON _il.[Invoice_id] = _i.[Id]
            ...
            Property(x => x.HomeCurrencyValue, map => map.Formula(@"(
                --HomeCurrencyValue
	            SELECT TOP 1
		            CASE _c.[Currency_id]
